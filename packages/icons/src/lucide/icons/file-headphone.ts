// Generated from @lucide/icons@1.33.0 by `bun run generate:lucide`.
// Do not edit by hand — see scripts/generate-lucide.mjs.

import createLucideIcon from "../createLucideIcon";
import type { IconNode } from "../types";

export const iconNode: IconNode = [
  ["path", { d: "M4 6.835V4a2 2 0 0 1 2-2h8a2.4 2.4 0 0 1 1.706.706l3.588 3.588A2.4 2.4 0 0 1 20 8v12a2 2 0 0 1-2 2h-.343", key: "1vfytu" }],
  ["path", { d: "M14 2v5a1 1 0 0 0 1 1h5", key: "wfsgrz" }],
  ["path", { d: "M2 19a2 2 0 0 1 4 0v1a2 2 0 0 1-4 0v-4a6 6 0 0 1 12 0v4a2 2 0 0 1-4 0v-1a2 2 0 0 1 4 0", key: "1etmh7" }],
];

const FileHeadphone = createLucideIcon("file-headphone", iconNode);

export default FileHeadphone;
